# íº€ Static Website Deployment â€“ Cloud Capstone Project

This project showcases the deployment of a **static website** on an **AWS EC2 instance** using **NGINX** and automated Bash scripting. It was completed as part of my Cloud Computing Capstone at **Techcrush Bootcamp**.

---

## í³Œ Objective

To build and deploy a static portfolio website on a cloud platform (AWS EC2), demonstrating core cloud concepts like:

- Linux server provisioning
- Web server configuration (NGINX)
- Secure SSH access
- Infrastructure automation with Bash
- GitHub-based version control and CI/CD (bonus)

---

## í·° Technologies & Tools Used

- **Amazon EC2 (Ubuntu 22.04)**
- **NGINX** â€“ Web Server
- **HTML5**, **CSS3**, and **JavaScript**
- **Bash scripting** â€“ for automated deployment
- **Git & GitHub**
- **GitHub Actions** (optional CI/CD)

---

## í·ª What I Did

âœ… Provisioned a new EC2 Ubuntu server  
âœ… Installed and configured NGINX  
âœ… Created a portfolio-style website (`index.html`, `style.css`, `script.js`)  
âœ… Wrote a full Bash script (`deploy.sh`) to automate the entire setup  
âœ… Pushed all project files to GitHub for version control  
âœ… (Optional) Setup GitHub Actions for continuous deployment

---

## í³ File Structure

capstone-project/
â”œâ”€â”€ index.html
â”œâ”€â”€ style.css
â”œâ”€â”€ script.js
â””â”€â”€ deploy.sh



---

## í¼ Live Preview (Demo)

> í´— http://13.51.163.195 
> *(Hosted on AWS EC2)*

---

## í·  Lessons Learned

- How to configure and secure a cloud server from scratch
- The role of NGINX in serving static content
- Writing idempotent and safe Bash scripts
- Using GitHub and GitHub Actions to automate workflows

---

## í¹‹ About Me

**Abdul Ganiyu Sumaila Ali**  
Aspiring Cloud Engineer | Techcrush Bootcamp Graduate  
í³§ [alisumaila.1000@gmail.com](mailto:alisumaila.1000@gmail.com)  
í´— [LinkedIn](https://www.linkedin.com/in/abdulganiyu)  
í²» [GitHub](https://github.com/G-abdul)

---

## í³¸ Screenshots

> _You can add screenshots of:_
- EC2 Instance Running
- Deployed website in browser
- GitHub Actions (if used)

---

## í³œ License

This project is for educational purposes as part of the Techcrush Cloud Bootcamp.

